...
type SettingsState = Omit<UserSettings, 'userId'>;
...
const Toggle = ({ label, description, checked, onChange }: { label: string; description: string; checked: boolean; onChange: (checked: boolean) => void }) => (
  <div className="flex items-center justify-between">
    ...
    <button onClick={() => onChange(!checked)} className={`w-12 h-6 rounded-full flex items-center transition-colors ${checked ? 'bg-accent' : 'bg-board'}`}>
...
const Select = ({ label, value, onChange, children }: { label: string; value: string; onChange: (value: string) => void; children: React.ReactNode }) => (
    <div className="flex items-center justify-between">
      <div className="font-medium text-primary">{label}</div>
      <select value={value} onChange={e => onChange(e.target.value)} className="p-2 rounded-md bg-board border border-secondary/20 text-primary">
        {children}
      </select>
    </div>
);
...
export function SettingsPage() {
  ...
  const handleUpdate = async (updatedFields: Partial<SettingsState>) => {
    if (!settings) return;
  ...
  const handleThemeChange = (theme: string) => {
    handleUpdate({ appearanceTheme: theme as UserSettings['appearanceTheme'] });
      ...
      <div className="space-y-8">
        <SettingsCard title="Notifications" icon={<Bell className="w-5 h-5 text-accent" />}>
          <Toggle
            ...
            onChange={(checked) => handleUpdate({ notificationsAssign: checked })}
          />
          <Toggle
            ...
            onChange={(checked) => handleUpdate({ notificationsDueDate: checked })}
          />
          <Toggle
            ...
            onChange={(checked) => handleUpdate({ notificationsComments: checked })}
          />
        </SettingsCard>
        ...
        <SettingsCard title="Account" icon={<User className="w-5 h-5 text-accent" />}>
           <div className="flex items-center justify-between">
                ...
                <button className="px-4 py-2 rounded-md border border-secondary/20 hover:bg-board text-sm font-semibold">
                  Export
                </button>
            </div>
            <div className="flex items-center justify-between">
