    ...
    <div className="w-[360px] shrink-0">
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2">
          ...
          <span className="px-2 py-0.5 text-xs rounded-full bg-blue-100 text-blue-700">{column.tasks?.length ?? 0}</span>
        </div>
      </div>
      <div ref={setNodeRef} className="space-y-3 min-h-[40px]">
        <SortableContext items={(column.tasks || []).map((t: any) => String(t.id))} strategy={verticalListSortingStrategy}>
          {(column.tasks || []).map((t: any) => (
            <div key={t.id} id={String(t.id)}>
              <TaskCard task={t} />
            </div>
          ))}
        </SortableContext>
      ...
      {!adding ? (
        ...
        <div className="mt-3 bg-white shadow rounded-lg p-2">
          <input
            autoFocus
            className="w-full px-3 py-2 rounded border border-secondary/30"
          ...
          <div className="flex gap-2 mt-2">
            ...
            <button
              className="px-3 py-1 rounded border border-secondary/30"
              onClick={() => {
                setTitle("")
                setAdding(false)
              }}
